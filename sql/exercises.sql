...
select payment_id, amount, payment_date from payment where Month(payment_date) >= 5 AND year(payment_date) >=2005 AND day(payment_date) >= 27;
...
select * staff where password is not null;
...
select * from payment where Date(payment_date) between ('2005-05-25 00:00:01') and ('2005-05-25 23:59:59');
...
select title, description, relesear_year from film where description like 'A Thoughtful%';
...
select payment, amount from payment limit 20 order by amount DESC;
...
-- This query returns the full list of names from customers, matching last names on the actors columns, and null values for the rest of the cells that do not have a match.
...
-- This query returns the full list of names from actor, matching last name on the customer columns, and null values for the rest of the cells that do not have a match.
...
-- This query returns the matching last name between customer and actor tables.
...
select city.city as city_name, country.country as country_name from city left join country on city.country_id = country.country_id;
